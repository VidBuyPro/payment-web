...
export default function CheckoutAdditionalPaymentView() {
  const [searchParams] = useSearchParams();
  const stripePromise = loadStripe(
    import.meta.env.VITE_STRIPE_PUBLIC_KEY || ''
  );

  const requiredParams = ['orderId', 'accountId', 'amountCents', 'cardRegion'];
  ...
  if (isInvalid) {
    return (
      <div className='text-red-600 text-center mt-10'>
        Unable to process your request at this time. found
        <br />
        Additional request not foud.
      </div>
    );
  }
  ...
  return (
    <div className='w-screen min-h-screen flex flex-col items-center pt-4 bg-white'>
      ...
      >
        <CheckoutAdditionalContent />
      </Elements>
    </div>
  );
}

function CheckoutAdditionalContent() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const dto = {
        ...
        amountCents: Number(searchParams.get('amountCents')),
        cardRegion: searchParams.get('cardRegion') as 'eu' | 'intl',
        paymentMethodId: pmRes.paymentMethod.id
      };

      const { data } = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/veripay/pay/additional`,
        dto,
        {
          headers: {
  ...
  const content = {
    ...
    payContent:
      searchParams.get('payContent') ??
      `Pay €${(Number(searchParams.get('amountCents')) / 100).toFixed(2)}`
  };
